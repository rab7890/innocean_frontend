sap.ui.define([
	...
	return Controller.extend("portal.controller.Bookmark", {
		...
		onInit: function() {
			...
			sap.ui.require(["portal.controller.Bookmark"], function(TableExampleUtils) {
				var oTb = oView.byId("infobar");
				oTb.addContent(new ToolbarSpacer());
				oTb.addContent(TableExampleUtils.createInfoButton("portal.controller.Bookmark"));
		...
		initSampleProductsModel: function() {
			var oData = jQuery.sap.sjax({
				url: sap.ui.require.toUrl("portal/model/tile.json"),
				dataType: "json"
			}).data;

			// prepare and initialize the rank property
			oData.Tile.forEach(function(oProduct) {
